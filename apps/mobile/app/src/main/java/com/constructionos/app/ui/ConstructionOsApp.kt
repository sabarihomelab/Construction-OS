...
private fun AuthenticatedScreen(
    ...
    Column(
        ...
        Text("Construction OS", style = MaterialTheme.typography.headlineMedium)
        Text(
            text = "Workspace authorized",
            style = MaterialTheme.typography.titleMedium,
            modifier = Modifier.padding(top = 8.dp),
        )
        Text(
            text = "Organization: ${context.organizationId}",
            modifier = Modifier.padding(top = 16.dp),
        )
        Text("Permissions: ${context.permissions.size}")
        Text("Mobile features: ${context.features.count { it.mobileEnabled }}")
        Button(
            onClick = onLogout,
