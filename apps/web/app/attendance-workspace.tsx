...
function canAccess(context: AccessContext, permission: string, projectId: string) {
  return (
...
export default function AttendanceWorkspace({
  ...
  const visibleEntries = useMemo(() => {
    ...
    return entries.filter((entry) => {
      ...
      return (
        (filter === "all" || entry.mark_status === filter) &&
  ...
  const editable = Boolean(
    register &&
      ["draft", "rejected"].includes(register.status) &&
      can("workforce.attendance.update", projectId),
  );
  ...
  const loadDay = useCallback(async (pid: string, date: string, shift: string) => {
    ...
    const [registerRows, rosterRows, wbsRows] = await Promise.all([
      ...
      api<RosterItem[]>(
        `/projects/${pid}/workforce/attendance/roster?attendance_date=${encodeURIComponent(date)}`,
      ),
    ...
    const selected =
      registerRows.find((row) => row.attendance_date === date && row.shift_code === shift) || null;
    if (!selected) {
      setRegister(null);
      setEntries([]);
      setSummary(null);
    ...
    const [detail, historyRows] = await Promise.all([
      ...
      api<HistoryEvent[]>(`/projects/${pid}/workforce/attendance/${selected.id}/history`).catch(
        () => [] as HistoryEvent[],
      ),
    ]);
    setRegister(detail);
    ...
    if (detail.status === "approved") {
      setSummary(
        await api<DPRSummary>(
          `/projects/${pid}/workforce/attendance/${detail.id}/dpr-summary`,
        ).catch(() => null as DPRSummary | null),
      );
    } else {
      setSummary(null);
    }
  ...
  const bootstrap = useCallback(async () => {
    ...
    try {
      ...
      const visible = projectRows.filter((project) =>
        canAccess(ctx, "workforce.attendance.view", project.id),
      );
      const selectedProject =
        initialProjectId && visible.some((project) => project.id === initialProjectId)
          ? initialProjectId
          : visible[0]?.id || "";
      ...
      if (initialRegisterId) {
        const detail = await api<AttendanceDetail>(
          `/projects/${selectedProject}/workforce/attendance/${initialRegisterId}`,
        );
        setAttendanceDate(detail.attendance_date);
        setShiftCode(detail.shift_code);
        await loadDay(selectedProject, detail.attendance_date, detail.shift_code);
      } else {
        await loadDay(selectedProject, attendanceDate, shiftCode);
      }
    } catch (reason) {
  ...
  }, [attendanceDate, initialProjectId, initialRegisterId, loadDay, shiftCode]);

  useEffect(() => {
    void bootstrap();
    // Bootstrap is intentionally one-time for the supplied deep-link context.
  ...
  async function startAttendance() {
    ...
    try {
      ...
      setMessage(
        `Attendance started with ${created.entries.length} active worker${created.entries.length === 1 ? "" : "s"}.`,
      );
  ...
  function updateEntry(assignmentId: string, patch: Partial<EditableEntry>) {
    setEntries((rows) =>
      rows.map((row) => (row.assignment_id === assignmentId ? { ...row, ...patch } : row)),
    );
  }
  ...
  async function saveEntries() {
    ...
    setMessage("");
    try {
      const updated = await api<AttendanceDetail>(
        `/projects/${projectId}/workforce/attendance/${register.id}/entries`,
        {
          method: "PUT",
          body: JSON.stringify({
      ...
      if (reason instanceof ApiError && reason.status === 409) {
        await loadDay(projectId, attendanceDate, shiftCode);
        setError(
          "This attendance register changed after you opened it. The latest revision has been loaded; review it and save again.",
        );
      } else {
        setError(reason instanceof Error ? reason.message : String(reason));
      }
  ...
  async function lifecycle(action: "submit" | "approve" | "reject") {
    ...
    setMessage("");
    try {
      await api<AttendanceRegister>(
        `/projects/${projectId}/workforce/attendance/${register.id}/${action}`,
        {
          method: "POST",
          body: JSON.stringify({
            expected_revision: register.revision,
      ...
      setMessage(
        action === "submit"
          ...
          : action === "approve"
            ? "Attendance approved and ready for DPR reuse."
      ...
      if (reasonValue instanceof ApiError && reasonValue.status === 409) {
        await loadDay(projectId, attendanceDate, shiftCode);
        setError(
          "This attendance register changed after you opened it. The latest revision has been loaded; review it and retry the workflow action.",
        );
      } else {
  ...
  if (loading) {
    return (
      <main className={styles.shell}>
        <div className={styles.loading}>Loading attendance workspace…</div>
      </main>
    );
  }
    ...
    <main className={styles.shell}>
      <header className={styles.header}>
        ...
        <div className={styles.headerActions}>
          <Link className={styles.secondaryButton} href="/workforce">Workforce</Link>
      ...
      <section className={styles.contextCard}>
        <label>
          ...
          <select
            value={projectId}
            disabled={busy}
            onChange={(event) => void changeContext(event.target.value, attendanceDate, shiftCode)}
          >
        ...
        <label>
          ...
          <input
            ...
            disabled={busy}
            onChange={(event) => void changeContext(projectId, event.target.value, shiftCode)}
          />
        </label>
        <label>
          Shift
          <select
            value={shiftCode}
            disabled={busy}
            onChange={(event) => void changeContext(projectId, attendanceDate, event.target.value)}
          >
      ...
      {!projectId ? (
        <section className={styles.emptyCard}>
          <h2>No accessible project</h2>
          <p>You need project permission <code>workforce.attendance.view</code> to use this workspace.</p>
        </section>
      ) : !register ? (
        <section className={styles.startCard}>
          <div>
            ...
            <p>
              {roster.length} active worker{roster.length === 1 ? "" : "s"} are available in the
              project roster for this date.
            </p>
          </div>
          {can("workforce.attendance.create", projectId) ? (
            <button disabled={busy} onClick={() => void startAttendance()} type="button">
              Start attendance
            </button>
          ) : (
        ...
        <>
          <section className={styles.registerBar}>
            <div>
              <span className={`${styles.status} ${styles[`status_${register.status}`] || ""}`}>
                {humanStatus(register.status)}
              </span>
            ...
            <div className={styles.actions}>
              ...
              {["draft", "rejected"].includes(register.status) &&
                can("workforce.attendance.submit", projectId) && (
                  <button
                    disabled={busy || !allMarked}
                    title={!allMarked ? "Mark every worker before submitting" : undefined}
              ...
              {register.status === "in_review" &&
                can("workforce.attendance.approve", projectId) && (
                  <>
                    <button disabled={busy} onClick={() => void lifecycle("approve")} type="button">
                      Approve
                    </button>
                    <button
          ...
          <section className={styles.metrics}>
            <button
              className={filter === "all" ? styles.metricActive : ""}
            ...
            <button
              className={filter === "present" ? styles.metricActive : ""}
              onClick={() => setFilter("present")}
              type="button"
            >
              <strong>{metrics.present}</strong><span>Present</span>
            </button>
            <button
              className={filter === "absent" ? styles.metricActive : ""}
              onClick={() => setFilter("absent")}
              type="button"
            >
              <strong>{metrics.absent}</strong><span>Absent</span>
            </button>
            <button
              className={filter === "half_day" ? styles.metricActive : ""}
              onClick={() => setFilter("half_day")}
              type="button"
            >
              <strong>{metrics.half_day}</strong><span>Half day</span>
            </button>
            <button
              className={filter === "not_marked" ? styles.metricWarn : ""}
              onClick={() => setFilter("not_marked")}
              type="button"
            >
              <strong>{metrics.not_marked}</strong><span>Not marked</span>
            </button>
          </section>
          ...
          <section className={styles.board}>
            <div className={styles.boardToolbar}>
              <div>
                ...
                <p>
                  {editable
                    ? "Edit marks, hours, cost code and location, then save."
                    : "This register is read-only at its current workflow status."}
                </p>
              </div>
              <div className={styles.toolbarControls}>
                <input
                  aria-label="Search crew"
                  placeholder="Search worker, number or trade"
                  value={search}
            ...
            <div className={styles.tableWrap}>
              <table>
                <thead>
                  <tr>
                    <th>Worker</th><th>Trade / role</th><th>Status</th><th>Regular</th><th>OT</th><th>WBS / cost code</th><th>Location</th><th>Notes</th>
                  </tr>
                </thead>
                <tbody>
                  {visibleEntries.map((entry) => {
                    const worker = rosterByAssignment.get(entry.assignment_id);
                    const workerName =
                      worker?.worker_name || String(entry.context_snapshot?.worker_name || "Worker");
                    const workerNumber =
                      worker?.worker_number || String(entry.context_snapshot?.worker_number || "");
                      ...
                      <tr key={entry.assignment_id}>
                          ...
                          <div className={styles.markGroup}>
                            {MARKS.map((mark) => (
                              <button
                                ...
                                disabled={!editable}
                                title={mark.label}
                                className={entry.mark_status === mark.value ? styles.markActive : ""}
                                onClick={() => setMark(entry.assignment_id, mark.value)}
                                type="button"
                              >
                                {mark.short}
                              </button>
                        ...
                        <td>
                          <input
                            ...
                            disabled={
                              !editable ||
                              ["not_marked", "absent", "leave", "weekly_off"].includes(entry.mark_status)
                            }
                            value={entry.regular_hours_text}
                            onChange={(event) =>
                              updateEntry(entry.assignment_id, { regular_hours_text: event.target.value })
                            }
                          />
                        </td>
                        <td>
                          <input
                            ...
                            value={entry.overtime_hours_text}
                            onChange={(event) =>
                              updateEntry(entry.assignment_id, { overtime_hours_text: event.target.value })
                            }
                          />
                        </td>
                        <td>
                          <select
                            disabled={!editable}
                            value={entry.wbs_code_id || ""}
                            onChange={(event) =>
                              updateEntry(entry.assignment_id, {
                                wbs_code_id: event.target.value || null,
                              })
                            }
                          >
                        ...
                        <td>
                          <input
                            ...
                            onChange={(event) =>
                              updateEntry(entry.assignment_id, { location: event.target.value || null })
                            }
                            placeholder="Zone / level"
                          />
                        </td>
                        <td>
                          <input
                            ...
                            value={entry.notes || ""}
                            onChange={(event) =>
                              updateEntry(entry.assignment_id, { notes: event.target.value || null })
                            }
                  ...
                  {visibleEntries.length === 0 && (
                    <tr><td colSpan={8} className={styles.emptyCell}>No workers match this filter.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          {register.status === "approved" && (
            <section className={styles.integrationCard}>
              <div>
                <p className={styles.eyebrow}>DPR INTEGRATION</p>
                <h2>Approved crew summary</h2>
                <p>This is the authoritative summary available for Daily Progress Report reuse.</p>
              </div>
              {summary?.rows.length ? (
            ...
            <section className={styles.historyCard}>
              ...
              <div className={styles.historyList}>
                {[...history].reverse().slice(0, 8).map((event) => (
                  <div key={event.id}>
                    <span>{humanStatus(event.event_type)}</span>
                    <small>
                      Revision {event.register_revision} · {new Date(event.created_at).toLocaleString()}
                    </small>
                  </div>
                ))}
              </div>
      ...
      {registers.length > 0 && (
        <footer className={styles.footerNote}>
          Project has {registers.length} attendance register{registers.length === 1 ? "" : "s"} recorded. The selected date/shift is shown above.
        </footer>
      )}
    </main>
  );
