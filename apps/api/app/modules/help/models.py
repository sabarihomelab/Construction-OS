...
class TenantKnowledgeSource(UUIDTimestampMixin, Base):
    ...
    kind: Mapped[KnowledgeSourceKind] = mapped_column(Enum(KnowledgeSourceKind, native_enum=False, values_callable=enum_values))
    status: Mapped[KnowledgeSourceStatus] = mapped_column(Enum(KnowledgeSourceStatus, native_enum=False, values_callable=enum_values), default=KnowledgeSourceStatus.PENDING)
    file_asset_id: Mapped[UUID | None] = mapped_column(Uuid, nullable=True)
    source_version: Mapped[int] = mapped_column(Integer, default=1)
    required_permission_key: Mapped[str | None] = mapped_column(ForeignKey("permissions.key", ondelete="RESTRICT"), nullable=True)
...
class TenantKnowledgeChunk(UUIDTimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("source_id", "source_version", "ordinal", name="uq_tenant_knowledge_chunks_version_ordinal"),
        ...
        CheckConstraint("character_count >= 0", name="ck_tenant_knowledge_chunks_character_count"),
