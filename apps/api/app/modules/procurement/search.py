...
from app.modules.procurement.sourcing_models import RequestForQuotation, VendorQuotation
...
async def rfq_search_projection(db: AsyncSession, organization_id: UUID, entity_id: str) -> SearchProjection | None:
    try:
        row_id = UUID(entity_id)
    except ValueError:
        return None
    row = await db.scalar(select(RequestForQuotation).where(RequestForQuotation.id == row_id, RequestForQuotation.organization_id == organization_id))
    if row is None:
        return None
    return SearchProjection(entity_type="procurement_rfq", entity_id=str(row.id), entity_version=row.revision, required_permission_key="procurement.purchase_order.view", scope_type="project", scope_id=str(row.project_id), title=row.title, subtitle=f"{row.number} · {row.status.value}", body=row.notes or "", keywords=[row.number, row.status.value], route_hint=f"/projects/{row.project_id}/procurement/rfqs/{row.id}", source_updated_at=row.updated_at)


async def vendor_quotation_search_projection(db: AsyncSession, organization_id: UUID, entity_id: str) -> SearchProjection | None:
    try:
        row_id = UUID(entity_id)
    except ValueError:
        return None
    row = await db.scalar(select(VendorQuotation).where(VendorQuotation.id == row_id, VendorQuotation.organization_id == organization_id))
    if row is None:
        return None
    return SearchProjection(entity_type="vendor_quotation", entity_id=str(row.id), entity_version=row.revision, required_permission_key="procurement.purchase_order.view", scope_type="project", scope_id=str(row.project_id), title=f"Vendor Quotation {row.number}", subtitle=row.status.value, body=row.notes or "", keywords=[row.number, row.status.value, row.currency_code, row.supplier_reference or ""], route_hint=f"/projects/{row.project_id}/procurement/rfqs/{row.rfq_id}/quotations/{row.id}", source_updated_at=row.updated_at)


for entity_type, provider in (
    ("purchase_requisition", requisition_search_projection),
    ("purchase_order", purchase_order_search_projection),
    ("goods_receipt", goods_receipt_search_projection),
    ("procurement_rfq", rfq_search_projection),
    ("vendor_quotation", vendor_quotation_search_projection),
):
