...
class DrawingSet(UUIDTimestampMixin, Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            name="fk_drawing_sets_project_org",
            ondelete="CASCADE",
        ),
    ...
    status: Mapped[DrawingSetStatus] = mapped_column(
        Enum(DrawingSetStatus, native_enum=False, values_callable=enum_values),
        default=DrawingSetStatus.ACTIVE,
    )
...
class DrawingSheet(UUIDTimestampMixin, Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["drawing_set_id", "organization_id"],
            ["drawing_sets.id", "drawing_sets.organization_id"],
            name="fk_drawing_sheets_set_org",
            ondelete="CASCADE",
        ),
...
class DrawingRevision(UUIDTimestampMixin, Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["sheet_id", "organization_id"],
            ["drawing_sheets.id", "drawing_sheets.organization_id"],
            name="fk_drawing_revisions_sheet_org",
            ondelete="CASCADE",
        ),
        ForeignKeyConstraint(
            ["file_version_id", "organization_id"],
            ["file_versions.id", "file_versions.organization_id"],
            name="fk_drawing_revisions_file_org",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["document_revision_id", "organization_id"],
            ["document_revisions.id", "document_revisions.organization_id"],
            name="fk_drawing_revisions_document_revision_org",
            ondelete="RESTRICT",
        ),
    ...
    status: Mapped[DrawingRevisionStatus] = mapped_column(
        Enum(DrawingRevisionStatus, native_enum=False, values_callable=enum_values),
        default=DrawingRevisionStatus.DRAFT,
    )
...
class DrawingRenderPackage(UUIDTimestampMixin, Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["drawing_revision_id", "organization_id"],
            ["drawing_revisions.id", "drawing_revisions.organization_id"],
            name="fk_drawing_render_packages_revision_org",
            ondelete="CASCADE",
        ),
        UniqueConstraint(
            "drawing_revision_id", "render_version", name="uq_drawing_render_packages_revision_version"
        ),
...
class DrawingCalibration(UUIDTimestampMixin, Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["drawing_revision_id", "organization_id"],
            ["drawing_revisions.id", "drawing_revisions.organization_id"],
            name="fk_drawing_calibrations_revision_org",
            ondelete="CASCADE",
        ),
        UniqueConstraint("id", "organization_id", name="uq_drawing_calibrations_id_org"),
        UniqueConstraint(
            "drawing_revision_id", "version", name="uq_drawing_calibrations_revision_version"
        ),
...
class DrawingMarkup(UUIDTimestampMixin, Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["drawing_revision_id", "organization_id"],
            ["drawing_revisions.id", "drawing_revisions.organization_id"],
            name="fk_drawing_markups_revision_org",
            ondelete="CASCADE",
        ),
    ...
    organization_id: Mapped[UUID] = mapped_column(Uuid, index=True)
    drawing_revision_id: Mapped[UUID] = mapped_column(Uuid, index=True)
    markup_type: Mapped[DrawingMarkupType] = mapped_column(
        Enum(DrawingMarkupType, native_enum=False, values_callable=enum_values)
    )
...
class DrawingMeasurement(UUIDTimestampMixin, Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["drawing_revision_id", "organization_id"],
            ["drawing_revisions.id", "drawing_revisions.organization_id"],
            name="fk_drawing_measurements_revision_org",
            ondelete="CASCADE",
        ),
        ForeignKeyConstraint(
            ["calibration_id", "organization_id"],
            ["drawing_calibrations.id", "drawing_calibrations.organization_id"],
            name="fk_drawing_measurements_calibration_org",
            ondelete="RESTRICT",
        ),
    ...
    measurement_type: Mapped[DrawingMeasurementType] = mapped_column(
        Enum(DrawingMeasurementType, native_enum=False, values_callable=enum_values)
    )
    geometry: Mapped[dict[str, object]] = mapped_column(JSONB)
...
class DrawingPin(UUIDTimestampMixin, Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["drawing_revision_id", "organization_id"],
            ["drawing_revisions.id", "drawing_revisions.organization_id"],
            name="fk_drawing_pins_revision_org",
            ondelete="CASCADE",
        ),
    ...
    organization_id: Mapped[UUID] = mapped_column(Uuid, index=True)
    drawing_revision_id: Mapped[UUID] = mapped_column(Uuid, index=True)
    pin_type: Mapped[DrawingPinType] = mapped_column(
        Enum(DrawingPinType, native_enum=False, values_callable=enum_values)
    )
...
class DrawingComparison(UUIDTimestampMixin, Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["drawing_revisions.id", "drawing_revisions.organization_id"],
            name="fk_drawing_comparisons_base_org",
            ondelete="CASCADE",
        ),
        ForeignKeyConstraint(
            ["compare_revision_id", "organization_id"],
            ["drawing_revisions.id", "drawing_revisions.organization_id"],
            name="fk_drawing_comparisons_compare_org",
            ondelete="CASCADE",
        ),
        UniqueConstraint(
            "base_revision_id", "compare_revision_id", name="uq_drawing_comparisons_pair"
        ),
        CheckConstraint(
            "base_revision_id <> compare_revision_id", name="ck_drawing_comparisons_distinct"
        ),
    ...
    status: Mapped[DrawingComparisonStatus] = mapped_column(
        Enum(DrawingComparisonStatus, native_enum=False, values_callable=enum_values),
        default=DrawingComparisonStatus.QUEUED,
    )
